    ...
    public ExecPlan plan(DeleteStmt deleteStatement, ConnectContext session) {
        ...
        try {
            ...
            OptExpression optimizedPlan = optimizer.optimize(
                    ...
                    new ColumnRefSet(logicalPlan.getOutputColumn().subList(logicalPlan.getOutputColumn().size()-2,logicalPlan.getOutputColumn().size())),
                    columnRefFactory);
            ExecPlan execPlan = PlanFragmentBuilder.createPhysicalPlan(optimizedPlan, session,
                    logicalPlan.getOutputColumn().subList(logicalPlan.getOutputColumn().size()-2,logicalPlan.getOutputColumn().size()), columnRefFactory,
