...
#include "iceberg_table_merge_sink_operator.h"
...
namespace starrocks::pipeline {
    ...
    Status IcebergTableMergeSinkOperator::prepare(RuntimeState* state) {
        RETURN_IF_ERROR(Operator::prepare(state));
        return Status::OK();
    }

    void IcebergTableMergeSinkOperator::close(RuntimeState* state) {
    ...
    bool IcebergTableMergeSinkOperator::need_input() const {
        for (const auto& writer : _partition_writers) {
    ...
    bool IcebergTableMergeSinkOperator::is_finished() const {
    ...
    Status IcebergTableMergeSinkOperator::set_finishing(RuntimeState* state) {
    ...
    bool IcebergTableMergeSinkOperator::pending_finish() const {
        return !is_finished();
    }

    Status IcebergTableMergeSinkOperator::set_cancelled(RuntimeState* state) {
        return Status::OK();
    }

    StatusOr<ChunkPtr> IcebergTableMergeSinkOperator::pull_chunk(RuntimeState* state) {
        return Status::InternalError("Shouldn't pull chunk from iceberg table sink operator");
    }

    Status IcebergTableMergeSinkOperator::push_chunk(RuntimeState* state, const ChunkPtr& chunk) {
        TableInfo tableInfo;
    ...
    std::string IcebergTableMergeSinkOperator::_get_partition_location(const std::vector<std::string>& names,
    ...
    IcebergTableMergeSinkOperatorFactory::IcebergTableMergeSinkOperatorFactory(int32_t id, FragmentContext* fragment_ctx,
                                                                     vector<TExpr> t_output_expr,
                                                                     IcebergTableDescriptor* iceberg_table,
                                                                     const TIcebergTableSink& thrift_sink,
                                                                     std::vector<ExprContext*> partition_expr_ctxs)
            : OperatorFactory(id, "iceberg_table_merge_sink", Operator::s_pseudo_plan_node_id_for_final_sink),
    ...
    Status IcebergTableMergeSinkOperatorFactory::prepare(RuntimeState* state) {
    ...
    void IcebergTableMergeSinkOperatorFactory::close(RuntimeState* state) {
        Expr::close(_partition_expr_ctxs, state);
        Expr::close(_output_expr_ctxs, state);
        OperatorFactory::close(state);
    }

    std::vector<parquet::FileColumnId> IcebergTableMergeSinkOperatorFactory::generate_parquet_field_ids(
...
#define MERGE_STATS_CASE(ParquetType)                                                                              \
    ...
    void merge_stats_new(const std::shared_ptr<::parquet::Statistics>& left,
    ...
    void calculate_column_stats_new(const std::shared_ptr<::parquet::FileMetaData>& meta, TIcebergColumnStats& t_column_stats) {
            ...
            for (int rg_idx = 0; rg_idx < meta->num_row_groups(); rg_idx++) {
                ...
                if (column_chunk_meta->is_stats_set()) {
                    ...
                    if (!column_stats.count(field_id)) {
                        ...
                        merge_stats_new(column_stats[field_id], column_stat);
                    }
                }
            }
        }
    ...
    void IcebergTableMergeSinkOperator::add_iceberg_commit_info(starrocks::parquet::AsyncFileWriter* writer,
                                                           RuntimeState* state) {
        TIcebergColumnStats iceberg_column_stats;
        calculate_column_stats_new(writer->metadata(), iceberg_column_stats);
        ...
        iceberg_data_file.__set_is_pos_delete(true);
    ...
    Status IcebergTableMergeSinkOperator::partition_value_to_string(Column* column, std::string& partition_value) {
        auto v = column->get(0);
        if (column->is_date()) {
